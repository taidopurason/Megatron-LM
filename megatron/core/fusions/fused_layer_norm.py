...
class FusedLayerNorm:
    """
    A conditional wrapper to initialize an instance of Apex's
    `LayerNorm` or `RMSNorm` based on input
    """

    def __new__(cls, config: TransformerConfig, hidden_size: int, eps: float = 1e-5):
        if config.normalization == "LayerNorm":
            instance = _FusedLayerNorm(config, hidden_size, eps)
        elif config.normalization == "RMSNorm":
            instance = _FusedRMSNorm(config, hidden_size, eps)
        else:
...
class _FusedLayerNorm(torch.nn.Module):
...
class _FusedRMSNorm(torch.nn.Module):
    ...
    def __init__(
        ...
        if persist_layer_norm:
            warnings.warn("Persistent Layer Norm is not supported for RMSNorm this version of Megatron.")
